# 📖 App Store & Google Play — Описание и Метаданные (v2.0)
...
Текст полностью проверен на лимиты символов Apple App Store Connect и Google Play Console и очищен от несовместимых символов.

---

## 📱 App Store Metadata (iOS)

### 1. Название (Title, max 30 chars):
* **EN**: `Luys: Armenian Bible & Widget` (29)
* **HY**: `Luys: Աստվածաշունչ և Վիդջեթ` (27)
* **RU**: `Luys: Армянская Библия` (22)

### 2. Подзаголовок (Subtitle, max 30 chars):
* **EN**: `Holy Bible, Narekatsi & Widget` (30)
* **HY**: `Սուրբ Գիրք, Նարեկացի և Վիդջեթ` (29)
* **RU**: `Библия, Нарекаци и Виджеты` (26)

### 3. Ключевые слова (Keywords, max 100 chars):
* **EN**: `bible,armenian bible,astvatsashunch,lock screen widget,narek,narekatsi,christian,verse,widget,quiz` (98)
* **HY**: `աստվածաշունչ,նարեկացի,վիդջեթ,աղոթք,հայերեն,սուրբ գիրք,նարեկ,քրիստոնեություն,լույս` (82)
* **RU**: `библия,армянская библия,нарекаци,виджет,локскрин,молитва,нарек,стих дня,христианство` (86)

### 4. Промо-текст (Promotional Text, max 170 chars):
* **EN**: `Daily inspiring Bible verses directly on your iPhone Lock Screen!`
* **HY**: `Աստվածաշնչի ոգեշնչող տողեր ձեր iPhone-ի կողպեքրանին ամեն օր:`
* **RU**: `Вдохновляющие стихи из Библии на экране блокировки вашего iPhone каждый день!`

---

## 🌐 Основное объединенное описание (Description, max 4000 chars):
...
## 📝 Текст для поля "What's New in This Version" (v2.0):

### Английский (English):
```text
- Armenian Bible Premium: StoreKit 2 subscriptions & Grandfathering for early adopters
...
### Армянский (Հայերեն):
```text
- Armenian Bible Premium: Բաժանորդագրություն և անվճար հասանելիություն նախկին գնորդների համար
...
### Русский (Русский):
```text
- Armenian Bible Premium: Подписки StoreKit 2 и пожизненный Premium для ранних покупателей
